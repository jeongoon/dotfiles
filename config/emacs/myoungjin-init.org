...
** Diminish / Minor-mode Display 
 ...
 #+BEGIN_SRC emacs-lisp
   ...
   ;;
   (setq eldoc-minor-mode-string (if (myjin/display-graphic-p) " 📚" " Ed"))
  ...
  (defun resize-minibuffer-window (&optional greeting-message)
    ...
    (let* ((minibuffer-orig-height (window-size (minibuffer-window)))
      ...
      (unless (= 0 delta)
        ...
        ;; 메시지는 수동으로 호출했을 때만 표시
        (when (and greeting-message (called-interactively-p 'interactive))
...
#+RESULTS:
: #[nil ((redraw-display) (resize-minibuffer-window)) nil nil nil nil]
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package rainbow-mode
    ...
    :config
    (setq rainbow-x-colors nil))  ; X11 컬러 이름 비활성화 (선택사항)
#+END_SRC
...
*** DONE Implement double C-s behavior for repeating previous search
CLOSED: [2025-11-16 Sat]
:LOGBOOK:
- State "DONE"       from              [2025-11-16 Sat] \\
  Implemented consult-line DWIM (Do What I Mean) functionality that mimics 
  swiper's behavior. When C-s is pressed twice within 0.5 seconds, it 
  automatically reuses the previous search term instead of starting a new 
  empty search. This improves workflow efficiency by reducing repetitive typing.
  
  Key implementation details:
  - Uses `consult--line-history` to retrieve the last search term
  - Safely checks if history exists using `boundp` to prevent errors
  - Combines time condition AND history existence in `use-last-search`
  - Tracks the time between consecutive C-s presses
  - Automatically inserts previous search when called rapidly
  - Works both in minibuffer and regular buffers
:END:

The implementation allows for quick re-search without needing to retype 
search terms, which is particularly useful when searching for the same 
pattern multiple times across different locations in a file.

   - Modern completion framework replacing ivy/counsel/swiper
   - Vertico: vertical completion UI
   - Consult: practical commands with completion
   - Orderless: flexible completion style

** Configuration
#+BEGIN_SRC emacs-lisp
  ;; 1. Minibuffer completion framework
  (use-package vertico
    :ensure t
    :init
    (vertico-mode))
  ...
  (use-package consult
    ...
    :init
    ;; Variable to track the last consult-line invocation time
    (defvar my/last-consult-line-time 0)

    (defun my/consult-line-dwim ()
      "Consult-line with Do What I Mean behavior.
If called twice within 0.5s, reuse the previous search term."
      (interactive)
      (let* ((now (float-time))
             (time-diff (- now my/last-consult-line-time))
             ;; Safely retrieve search history
             (last-search (and (boundp 'consult--line-history)
                              consult--line-history
                              (car consult--line-history)))
             ;; Check both time condition AND history existence
             (use-last-search (and (< time-diff 0.5) last-search)))

        (setq my/last-consult-line-time now)

        (cond
         ;; Inside minibuffer
         ((minibufferp)
          (when use-last-search
            (delete-minibuffer-contents)
            (insert last-search)))

         ;; Regular buffer - reuse previous search
         (use-last-search
          (message "Repeating: %s" last-search)
          (consult-line last-search))

         ;; Normal search
         (t
          (consult-line)))))

    ;; Bind C-s to the DWIM function
    (global-set-key (kbd "C-s") 'my/consult-line-dwim))


  ;; 3. Improved sorting/filtering
  ...
  ;; 4. Auto-completion (replaces auto-complete)
  (use-package company
    :ensure t
    :init
    (global-company-mode)
  ...
  ;; Debug functions for checking history contents
  (defun my/test-search-ring ()
    "Check search-ring contents"
    (interactive)
    (message "search-ring: %S" search-ring)
    (message "First (car): %s" (car search-ring))
    (message "Second (cadr): %s" (cadr search-ring))
    (message "Third (caddr): %s" (caddr search-ring)))

  (defun my/check-history ()
    "Check various history variables"
    (interactive)
    (message "=== History Check ===")
    (message "search-ring: %S" search-ring)
    (message "regexp-search-ring: %S" regexp-search-ring)
    (message "minibuffer-history: %S" (take 5 minibuffer-history))
    (message "consult--line-history: %S" 
             (bound-and-true-p consult--line-history)))

  (global-set-key (kbd "C-c h") 'my/check-history)
  (global-set-key (kbd "C-c t") 'my/test-search-ring)

  ;; 5. org-roam integration
  (use-package consult-org-roam
    ...
    :bind
    (("C-c n f" . consult-org-roam-file-find)      ; Find files
     ("C-c n b" . consult-org-roam-backlinks)      ; View backlinks
     ("C-c n B" . consult-org-roam-backlinks-recursive) ; Recursive backlinks
     ("C-c n s" . consult-org-roam-search)         ; Search content
     ("C-c n t" . consult-org-roam-forward-links))) ; Forward links
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq myjin/org-bullets-bullet-list (cons (if (myjin/display-graphic-p) "⚝" "❃")
                                                myjin/org-bullets-bullet-list-common))

  (setq org-bullets-bullet-list myjin/org-bullets-bullet-list)
  (setq org-ellipsis (if (myjin/display-graphic-p) "⮯"  " »"))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;  (setq business-photo-auto-convert nil))
    (use-package business-photo-upload
      ...
      :custom
      ;; === 파일 위치 설정 ===
      (business-photo-org-directory "~/gtd/")     ; org 디렉토리
      (business-photo-log-filename "hair-studio-celine-media-upload.org")       ; 파일명
      ;; === Capture 설정 ===
      (business-photo-capture-key "B")                    ; Capture 키 (B가 충돌시)
      (business-photo-auto-convert t)                     ; 자동 변환 여부
      ;; === 이미지 변환 설정 ===
      ;;(business-photo-apple-max-size 3000)               ; Apple 최대 크기 (픽셀)
      (business-photo-apple-quality 100)                  ; Apple JPEG 품질 (1-100)
      (business-photo-google-quality 100)                 ; Google JPEG 품질 (1-100)
      (business-photo-show-images nil)        ; 자동 이미지 표시
      (business-photo-image-width 250)      ; 표시 크기 (픽)셀
      (business-photo-search-directories
       '("~/Pictures/iCloud/"))
      ;;:bind
      ;; 전역 키바인딩
      ;;(("C-c p u" . my/business-mark-uploaded)           ; 업로드 체크
      ;;("C-c p d" . my/business-convert-directory))      ; 디렉토리 변환
      ;;:hook
      ;; org-mode에서만 활성화되는 키바인딩
      ;;(org-mode . (lambda ()
...
#+begin_src emacs-lisp
  ...
  (use-package flyspell
    ...
    (:map flyspell-mode-map
          ;; 기본 키바인딩이 다른 메이저 모드와 충돌할 수 있어서 제거
          ("C-;" . nil)
          ("C-." . nil)
          ("C-," . nil))
    :custom
    (flyspell-issue-message-flag nil)  ; 성능 향상을 위해 메시지 출력 비활성화
    (flyspell-issue-welcome-flag nil))
#+end_src

#+begin_src elisp
  ;; 팝업 인터페이스 개선을 위한 popup 패키지 설정
  (use-package popup
    :ensure t)

  ;; flyspell-popup 설정 (마우스 우클릭 대신 팝업 인터페이스 사용)
...
#+begin_src emacs-lisp
  ...
  ;; 비즈니스 전기세 템플릿 생성 함수 (schedule 포함)
  ...
  (defun myjin/add-bill-capture-templates ()
    ...
    (let ((bill-templates
           '(("b" "Bills")
             ...
             ("bb" "Business Bills")       ; bbe, bbn을 위한 그룹 헤더
             ...
             ;; L'oreal 관련
             ("bl" "L'oreal")              ; bl, bln을 위한 그룹 헤더  
             ("blp" "L'oreal Payment" item  ; 공백 추가로 구분
