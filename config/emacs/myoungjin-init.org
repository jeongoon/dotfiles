...
CLOSED: [2025-11-16 Sun]
:LOGBOOK:
- State "DONE"       from              [2025-11-16 Sun] \\
...
If called twice within 0.5s, reuse the previous search term."
          ...
          (let* ((now (float-time))
                 ...
                 (use-last-search (< time-diff 0.5))
                 ;; Retrieve from consult--line-history
                 (last-search (car consult--line-history)))
            ...
            (cond
             ;; When in minibuffer
             ((minibufferp)
              (when (and use-last-search last-search)
                (delete-minibuffer-contents)
                (insert last-search)))
             ;; When in regular buffer
             (use-last-search
              (when last-search
